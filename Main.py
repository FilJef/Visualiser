...
NewsArticles = pandas.read_sql_query("SELECT * FROM articles ORDER BY date ASC", index_col="date", con=gcloudcon)
...
if x is 0:
    ...
    ax1.plot(StockData["Unixdate"], StockData["1. open"])
    ax1.plot(StockData["Unixdate"], StockData["2. high"])
    ax1.plot(StockData["Unixdate"], StockData["3. low"])
    ...
    ax2.plot(NewsArticles["Unixdate"], NewsArticles["Sentiment"], 'ro',  picker=5)
